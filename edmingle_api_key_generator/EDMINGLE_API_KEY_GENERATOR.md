...
| `scripts/edmingle_api_key_settings.py` | Loads settings from `credentials.yaml` and this folder's `notifications.yaml`. |
...
| `scripts/notifications.yaml` | This folder's SMTP/recipient config (restricted permissions, not committed). |
