...
- Check the date window in `scripts/ip_driven_country_data_config.json`.
...
## Check
- `output/user_country_list_checkpoint.json` shows Stage 1 progress (last seen: page 124, 61,722 rows — **partly done, not finished**).
...
## If it stops
Re-run the same command; Stage 1 continues from its checkpoint. Stage 2 "No --input given…" → put a `Student-Export*.csv` in `input/`. Stage 3 "not found" → run Stages 1–2 first.
