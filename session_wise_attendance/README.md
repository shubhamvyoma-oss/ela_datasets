...
Credentials/notifications loading and the SMTP connect-and-send mechanics now come from the shared `../../common.py` (see its docstring) rather than pipeline-local copies -- `pipeline_common.py` keeps the same `load_config()` / `_merge_credentials()` / `_merge_notifications()` / `send_run_report()` function names and signatures (and the same `config["api_key"]`/`config["org_id"]`/`config["orgid"]`/`config["institute_id"]`/`config["smtp"]` shape) for backward compatibility, just delegating internally. The pipeline's own rate limiter (`pipeline_common.RateLimiter`, a flat per-call delay) is a different shape from `common.py`'s rolling-window `RollingRateLimiter` and was deliberately left as-is.
